<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST'){
        ...
        if ($checkNum != 1){
            ...
            if(!$mail->send()) {
                if($polaczenie->query("INSERT INTO logs VALUES (NULL, 'Gracz o id = ". $idGracza." wyslal prosbe o zmiane hasla - na jego adres emial NIE zostala wyslana wiadomosc z linkiem - blad systemu', now())")){
                $msg2 = ["typeEr" => "error", "text" => "Wiadomość nie została wysłana: ". $mail->ErrorInfo];
                  }
            } else {
                if($polaczenie->query("INSERT INTO logs VALUES (NULL, 'Gracz o id = ".$idGracza." wyslal prosbe o zmiane hasla - na jego adres emial zostala wyslana wiadomosc z linkiem', now())")){
